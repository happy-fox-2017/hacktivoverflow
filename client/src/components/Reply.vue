  ...
  <div class="row">
    <div class="col-md-1">
      <div class="text-center" title="This response is useful">
        ...
        <i v-if="upvoted" class="fa fa-caret-up" aria-hidden="true" style="font-size: 3em; color: blue; cursor: pointer"></i>
      </div>
      <div class="text-center" title="This response is useful">
...
export default {
  ...
  methods: {
    getResponse () {
      var self = this
      this.axios.get(`http://localhost:3000/questions/${self.parent}/reply/${self.repid}`)
      .then(response => {
    ...
    doUpvote () {
      var self = this
      this.axios.put(`http://localhost:3000/questions/${self.parent}/reply/${self.response._id}/upvote`, {
        token: self.token
      })
      .then(response => {
        if (self.downvoted) {
    ...
    doDownvote () {
      var self = this
      this.axios.put(`http://localhost:3000/questions/${self.parent}/reply/${self.response._id}/downvote`, {
        token: self.token
      })
      .then(response => {
        if (self.upvoted) {
    ...
    removeItem () {
      ...
      if (window.confirm('Are you sure you want to delete this thread? (cannot be undone)')) {
        this.axios.delete(`http://localhost:3000/questions/${self.parent}/reply/${self.response._id}`)
        .then(response => {
